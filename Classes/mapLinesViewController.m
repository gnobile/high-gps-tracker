...
static mapLinesViewController *shAcc = nil;

+ (mapLinesViewController *) shared {
    if (!shAcc) {
        shAcc = [[mapLinesViewController alloc] init];
    }
    return shAcc;
}
...
-(void)generaMappa {
	...
	if ((pointStrings.count <2) || !([[NSFileManager defaultManager] fileExistsAtPath:fullPathToFile]))
